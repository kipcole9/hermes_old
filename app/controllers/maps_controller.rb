class MapsController < AssetsController
  ...
  def world
    @map = GMap.new("_largeMap")
  	...
  	markers = []
  	galleries = Gallery.viewable_by(current_user).find(:all, :conditions => "latitude IS NOT NULL AND longitude IS NOT NULL and latitude <> 0 and longitude <> 0")
  	galleries.each do |g|
  	  info_window = render_to_string :partial => "infoWindow", :locals => {:image_name => g.popular_image(current_user).name, :asset => g}
  	  markers << GMarker.new([g.latitude, g.longitude], :info_window => info_window, :title => g.title, :description => g.description)
	  end
