  ...
  module Scaffolding # :nodoc:
          ...
          private
            ...
            def render_invocation_scaffold(action)
              ...
              if self.active_layout.nil?
                render({:text=>content, :layout=>sscaffold_path("layout"), :status=>200})
              else
                render({:text=>content, :layout=>self.active_layout, :status=>200})
              end
            end
    ...
    module Helpers # :nodoc:
      ...
      def service_method_list(service)
        ...
        names = service.api_methods.keys.sort
        methods = names.collect do |name|
          content_tag("li","["+ 
              link_to(name, :action => action, :service => service.name, :method =>name )+
              "]"+service.api_methods[name].to_s)
        end
