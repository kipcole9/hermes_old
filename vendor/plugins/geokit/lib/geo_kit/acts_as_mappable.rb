  ...
  module ActsAsMappable 
    ...
    module ClassMethods # :nodoc:
      ...
      def acts_as_mappable(options = {})
        ...
        cattr_accessor :delegate, :delegate_table  # To allow delegation to an included table
        self.distance_column_name = options[:distance_column_name]  || 'distance'
        self.default_units = options[:default_units] || GeoKit::default_units
        self.default_formula = options[:default_formula] || GeoKit::default_formula
        self.delegate_table = options[:delegate] ? options[:delegate].to_s.pluralize : table_name
        self.delegate = options[:delegate]
        self.lat_column_name = options[:lat_column_name] || 'lat'
        self.lng_column_name = options[:lng_column_name] || 'lng'
        self.qualified_lat_column_name = "#{self.delegate_table}.#{lat_column_name}"
        self.qualified_lng_column_name = "#{self.delegate_table}.#{lng_column_name}"
    ...
    module InstanceMethods #:nodoc:    
      ...
      module SingletonMethods # :nodoc:
        ...
        def prepare_for_find_or_count(action, args)
          ...
          add_delegate_to_args(options) if self.delegate
        ...
        # Add an include for our delegate table if there is one.
        def add_delegate_to_args(args)
          return if args[:include] && args[:include] == self.delegate
          args[:include] = self.delegate unless args[:include]
          if args[:include].is_a?(Array)
            args[:include].push self.delegate
          end
        end
