...
# examples of usage.
#
# Modified January 2008 by Kip Cole to include partial templates for Rails 2
...
#  end
  ...
  module ClassMethods
    ...
    # Wrap this around each sidebar giving a standard look and feel to each
    # sidebar component
    def sidebars_default_layout=(layout)
      @@sidebars_default_layout = layout
    end
    
    def sidebars_default_layout
      @@sidebars_default_layout ||= sidebars_path.split('/').last + "/sidebars_default_layout"
    end
    ...
    def sidebar_layout(sidebar)
      sb = find_sidebar(sidebar)
      sb ? (sb[:layout] || sidebars_default_layout) : sidebars_default_layout
    end
    ...
    def normalize_sidebar_options(options)
      ...
      raise UnknownSidebarPartial unless options[:component] || File.exist?("#{sidebars_path}/_#{options[:id].to_s}.rhtml")|| File.exist?("#{sidebars_path}/_#{options[:id].to_s}.html.erb")
      ...
      options[:display] = (options[:display] == true ? true : false) if options[:display]
      options
    end
  ...
  def sidebars_default_layout=(layout); self.class.sidebars_default(layout); end
  def sidebars_default_layout; self.class.sidebars_default_layout; end
  def sidebar_layout(obj); self.class.sidebar_layout(obj); end
    ...
    if current_sidebars = @controller.sidebars_after_conditions
      current_sidebars.map do |sb|
        if sb[:component]
          ...
          render :partial => "#{@controller.sidebars_path.split('/').last}/#{sb[:id].to_s}", :layout => "#{@controller.sidebar_layout(sb[:id])}"
        end
      end.join("\n")
    end
  end
